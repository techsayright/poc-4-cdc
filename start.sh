...
docker-compose exec -T ksqldb-cli ksql http://ksqldb-server:8088 <<-EOF
    ...
    CREATE STREAM courses_src_rekey WITH (PARTITIONS=4) AS \ SELECT * FROM courses_src PARTITION BY course_id;
    ...
    CREATE STREAM subjects_src_rekey WITH (PARTITIONS=4) AS \ SELECT * FROM subjects_src PARTITION BY subject_id;
    ...
    CREATE STREAM chapters_src_rekey WITH (PARTITIONS=4) AS \ SELECT * FROM chapters_src PARTITION BY chapter_id;
    ...
    CREATE STREAM subchapters_src_rekey WITH (PARTITIONS=4) AS \ SELECT * FROM subchapters_src PARTITION BY subchapter_id;
